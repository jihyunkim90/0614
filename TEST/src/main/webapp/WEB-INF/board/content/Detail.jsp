...
<body>
	...
	<table border="1">
		...
		<tr>
			<td>
				<!-- 좋아요 버튼 --> <c:if test="${lcount eq 0}">
					<input type="button"
						onclick="location.href='/board/content/like?pid=${nt.id}'"
						value="like it">
				</c:if> <c:if test="${lcount !=0 }">
	...
	<div>
		<input type="button"
			onclick="location.href='/board/content/list?p=${p}&q=${q}'"
			value="목록으로">
		<!-- 수정버튼 -->
		<c:if test="${sessionScope.userID==nt.writer_id}">
			<input type="button"
				onclick="location.href='modify?id=${nt.id}&q=${q}&f=${f}'"
				value="수정">
		</c:if>
		<c:if test="${sessionScope.userID!=nt.writer_id}">
