					...
					<ul class="nav flex-column">
						...
						<c:forEach var="mn" items="${menu}">
							<li class="nav-item"><a class="nav-link" aria-current="page"
								href="${root}/board/content/list?b=${mn.boardid}"> <c:choose>
										<c:when test="${mn.boardid eq 1}">
											<span data-feather="home"></span>
											<a href="${root}/board/list/?boardid=${mn.boardid}"></a>
										</c:when>
									...
									</c:choose> ${mn.boardName}
							</a></li>
						</c:forEach>
					</ul>
					<div>
						<button class="btn btn-primary" type="button"
							onclick="location.href='${root}/createBoard'">게시판생성</button>
					</div>
				</div>
			</nav>
		</div>
